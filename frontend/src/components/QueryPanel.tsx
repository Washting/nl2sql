...
import { Play, FileText, Lightbulb, ChevronDown, ChevronRight } from 'lucide-react';
    ...
    <div className="h-full flex flex-col bg-background overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-border/40 flex-shrink-0">
        <div className="flex items-center justify-between">
          <h2 className="text-cyan-600 dark:text-cyan-400 font-medium">智能问答区</h2>
          {selectedTable && (
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          {generatedSQL && (
            <Collapsible open={showSQL} onOpenChange={setShowSQL}>
              <CollapsibleTrigger className="flex items-center gap-2 w-full px-4 py-3 bg-card/50 hover:bg-card/80 rounded-lg border border-purple-500/20 hover:border-purple-500/40 transition-colors">
                {showSQL ? (
              ...
              <CollapsibleContent className="mt-2">
                <div className="bg-muted/50 rounded-lg border border-border/40 p-4 font-mono text-xs overflow-x-auto max-h-80 overflow-y-auto">
          ...
          {reasoning.length > 0 && (
            <Collapsible open={showReasoning} onOpenChange={setShowReasoning}>
              <CollapsibleTrigger className="flex items-center gap-2 w-full px-4 py-3 bg-card/50 hover:bg-card/80 rounded-lg border border-cyan-500/20 hover:border-cyan-500/40 transition-colors">
                {showReasoning ? (
              ...
              <CollapsibleContent className="mt-2">
                <div className="bg-muted/50 rounded-lg border border-border/40 p-4">
      ...
      <div className="border-t border-border/40 p-6 flex-shrink-0">
        <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              {exampleQuestions.map((question, idx) => (
                <Button
                  ...
                  className="text-xs bg-secondary/50 hover:bg-secondary text-foreground/80 border border-border/40 justify-start transition-colors"
                >
                  {question}
                </Button>
              ))}
          ...
          <Textarea
            ...
            disabled={!selectedTable}
            className="min-h-[100px] bg-card/50 border-border/40 text-foreground placeholder-muted-foreground resize-none focus:border-cyan-500/30"
          />
